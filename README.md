...
키워드 하나로 조사·이미지 준비·초안 작성·Chrome 입력까지 맡기고, 최종 게시 버튼은 사용자가 직접 누릅니다.
...
이미지는 Codex에서 연결된 생성 도구가 있으면 생성하고, 없으면 사용자 제공 파일을 사용합니다. Claude Code판은 이미지 생성 프롬프트를 만들고 사용자가 준비한 이미지로 이어갑니다. 파일이 없으면 원고를 보존하고 이미지 첨부만 대기합니다.
