...
정사각형 또는 4:5 이미지 1장을 반드시 준비하세요. 이미지 파일이 없으면 인스타그램·네이버 입력 단계로 넘어가지 않습니다. 파일이 없을 때는 생성용 추천 프롬프트를 먼저 제안하며, 실제 첨부는 사용자가 제공한 파일로만 진행합니다. 주소는 channels.md가 있으면 다시 묻지 않습니다.
